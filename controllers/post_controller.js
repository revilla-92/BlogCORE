...
exports.search = function(req, res, next) {
  ...
  q = q.replace(/\s+/g, '%');
        models.Post
        .findAll({where: ["title like ? OR body like ?", q, q], order: [['updatedAt','DESC']],
                  include: [ { model: models.User,
                               as: 'Author' }
                           ]})
        .success(function(posts) {
            var numero = posts.length;
            if(numero==0){
              req.flash('info', 'No se han encontrado post(s).');
            }
            else{
              req.flash('success', numero + ' post(s) encontrados.');
            }
          if(req.session.user){
